...
import { Check, X, Building, Users, Briefcase, Zap, Star, ShieldCheck, Loader2 } from 'lucide-react';
import RoleSelector from '../components/RoleSelector';
import { updateUserPlan } from '@/app/lib/actions/user';
...
export default function PricingClient({ allFeatures, allPlans, user }: PricingClientProps) {
    ...
    useEffect(() => {
        if (user && ['owner', 'client', 'agent', 'developer'].includes(user.role)) {
            setUserType(user.role as any);
        } else if (roleParam && ['owner', 'client', 'agent', 'developer'].includes(roleParam)) {
    ...
    const handlePlanAction = async (planName: string) => {
        if (user) {
            // User is logged in -> Switch Plan
            if (user.role !== userType) {
                // Should not happen if we lock role selector, but just in case
                alert("You cannot switch to a plan for a different role.");
                return;
            }

            if (confirm(`Are you sure you want to switch to the ${planName} plan?`)) {
                setLoadingPlan(planName);
                const res = await updateUserPlan(planName);
                setLoadingPlan(null);
                if (res.error) {
                    alert(res.error);
                } else {
            ...
            router.push(`/auth/signup?role=${userType}&plan=${encodeURIComponent(planName)}`);
        }
    };
    ...
    const isCurrentPlan = (planName: string) => {
        ...
        if (tier === 'pro' && (lowerName.includes('premium') || lowerName.includes('pro') || lowerName.includes('growth'))) return true;
        if (tier === 'enterprise' && (lowerName.includes('enterprise') || lowerName.includes('scale'))) return true;
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 py-20 px-4 sm:px-6 lg:px-8">
            <div className="max-w-7xl mx-auto">
                ...
                {/* If user is logged in, maybe disable role selector or show it locked? 
                    For now keeping it interactive but switching it resets view to that role 
                    (but user can't buy it if logged in as diff role). */}
                <RoleSelector
                    mode="selection"
                ...
                {user && user.role !== userType && (
                    <div className="text-center text-amber-400 mb-6 bg-amber-900/20 p-2 rounded-lg border border-amber-500/30 inline-block mx-auto w-full max-w-lg">
                        You are currently logged in as a <strong>{user.role}</strong>. Viewing plans for <strong>{userType}</strong>.
                    </div>
                )}
                ...
                <div className="flex flex-wrap justify-center gap-8">
                    {currentPlans.length === 0 ? (
                        ...
                        currentPlans.map((plan: any) => {
                            ...
                            return (
                                <div key={plan.name} className={`relative rounded-3xl p-8 transition-all duration-500 flex flex-col w-full sm:w-[calc(50%-1rem)] lg:w-[calc(25%-1.5rem)] min-w-[300px] max-w-[340px] group ${isCurrent
                                    ? 'bg-gradient-to-b from-emerald-900/40 to-slate-900 border-2 border-emerald-500 shadow-2xl shadow-emerald-500/20 scale-105 z-10'
                                    : plan.popular
                                    ...
                                    {isCurrent && (
                                        <div className="absolute top-0 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-gradient-to-r from-emerald-500 to-green-600 text-white text-xs font-bold uppercase tracking-widest px-4 py-1.5 rounded-full shadow-lg shadow-emerald-500/50 border border-emerald-400">
                                            Current Plan
                                        </div>
                                    )}
                                    ...
                                    <button
                                        ...
                                        disabled={isCurrent || (user && user.role !== userType) || isLoading}
                                        className={`w-full py-4 rounded-xl font-black text-lg transition-all duration-300 transform hover:-translate-y-1 active:scale-95 flex justify-center items-center gap-2 ${isCurrent
                                            ? 'bg-emerald-900/50 text-emerald-400 cursor-default border-2 border-emerald-800'
                                            : (user && user.role !== userType)
                                                ? 'bg-slate-800 text-gray-500 cursor-not-allowed border-2 border-slate-700'
                                                : plan.popular
                                    ...
                                    >
                                        ...
                                        {isCurrent ? 'Current Plan' : user ? 'Switch Plan' : 'Get Started Now'}
                                    </button>
                                </div>
                            );
                        })
