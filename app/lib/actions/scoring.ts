...
export async function calculateLeadScore(lead: LeadData): Promise<number> {
    ...
    // Move Urgency
    if (lead.move_urgency?.includes('Urgent') || lead.move_urgency === '< 1 month (Urgent)') {
        score += getWeight('urgency_urgent') || 20;
    } else if (lead.move_urgency?.includes('Moderate') || lead.move_urgency === '1-3 months (Moderate)') {
        score += getWeight('urgency_moderate') || 15;
    } else if (lead.move_urgency?.includes('Low') || lead.move_urgency === '> 3 months (Low)') {
        score += getWeight('urgency_low') || 10;
    }
    ...
    // --- Essential Lead Preference Scoring (For Public & Agent Leads) ---
    // Budget
    const maxB = Number(lead.budget_max) || 0;
    if (maxB > 0) {
        score += maxB >= 100000 ? 20 : 15;
    }
    ...
    if (lead.email && lead.email.trim().length >= 5) {
        score += 5;
    }
    ...
    // Public Form Engagement
    if (lead.source === 'Shared Link Form' || (lead as any).created_via === 'invite_form') {
        score += 10;
    }
