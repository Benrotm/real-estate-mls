...
export async function getUserProperties(filters?: any) {
    const supabase = await createClient();
    const { data: { user } } = await supabase.auth.getUser();
...
export async function createPropertyFromData(data: Partial<PropertyType>, sourceUrl?: string, adminBypassUserId?: string) {
    ...
    try {
        ...
        let propertyData: any = {
            ...
            // Specs
            rooms: data.rooms ? parseInt(String(data.rooms).replace(/\D/g, '')) || null : null,
            bedrooms: data.bedrooms ? parseInt(String(data.bedrooms).replace(/\D/g, '')) || null : null,
            bathrooms: data.bathrooms ? parseInt(String(data.bathrooms).replace(/\D/g, '')) || null : null,
            area_usable: data.area_usable ? parseFloat(String(data.area_usable).replace(/[^0-9.]/g, '')) || null : null,
            area_built: data.area_built ? parseFloat(String(data.area_built).replace(/[^0-9.]/g, '')) || null : null,
            area_terrace: data.area_terrace ? parseFloat(String(data.area_terrace).replace(/[^0-9.]/g, '')) || null : null,
            area_garden: data.area_garden ? parseFloat(String(data.area_garden).replace(/[^0-9.]/g, '')) || null : null,
            floor: (data.floor !== undefined && data.floor !== null) ? ((val) => isNaN(val) ? null : val)(parseInt(String(data.floor).replace(/^[^\d-]*(-?\d+).*$/, '$1'))) : null,
            total_floors: data.total_floors ? parseInt(String(data.total_floors).replace(/\D/g, '')) || null : null,
            year_built: data.year_built ? parseInt(String(data.year_built).replace(/\D/g, '')) || null : null,
