...
export default function AIPipelineClient({ initialUsers, initialRecommendation }: Props) {
    ...
    const CATEGORIES = [
        ...
        },
        {
            ...
            col2Title: 'Cereri În Așteptare / Suspendate',
            col3Title: 'Clienți Arhivați',
            active: cat3Users.filter(u => u.is_approved !== false && u.is_archived !== true),
            suspended: cat3Users.filter(u => u.is_approved === false && u.is_archived !== true),
        ...
        },
        {
            ...
            col2Title: 'Clienți Suspendați',
            col3Title: 'Arhivă Definitivă',
            active: cat4ArchivedUsers.filter(u => u.is_approved !== false),
            suspended: cat4ArchivedUsers.filter(u => u.is_approved === false),
            archived: cat4ArchivedUsers
        }
    ];
            ...
            <div className="space-y-1.5 pt-3 border-t border-slate-100">
                ...
                {colType === 'suspended' && (
                    <>
                        <button
                            onClick={() => handleApproveUser(user.id, user.is_approved)}
                            className="w-full py-2 px-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-xs font-black flex items-center justify-center gap-1.5 shadow-sm cursor-pointer"
                        >
                        ...
                        <div className="grid grid-cols-2 gap-1.5">
                            <button
                                onClick={() => setPreviewUser(user)}
                                className="py-1.5 px-2 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-lg text-[11px] font-bold flex items-center justify-center gap-1 border border-slate-200 cursor-pointer"
                            >
    ...
    return (
        <div className="flex flex-col h-full space-y-6">
            ...
            {/* Category Groups View - 3 Sub-Columns Grid (Activi, Suspendați, Arhivați) */}
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
                            ...
                            <div className="bg-emerald-50/40 rounded-xl border border-emerald-200/80 p-4 space-y-3">
                                ...
                                <div className="space-y-3 max-h-[600px] overflow-y-auto pr-1">
                                    {cat.active.length > 0 ? (
                                        cat.active.map(user => renderCard(user, 'active'))
                                    ) : (
                                        <div className="py-8 text-center text-slate-400 text-xs font-semibold italic">
                                            Niciun client activ în această secțiune.
                                        </div>
                                    )}
                                </div>
                            </div>

                            {/* Column 2: Suspendați / Acces Suspendat */}
                            <div className="bg-amber-50/50 rounded-xl border border-amber-200/80 p-4 space-y-3">
                                <div className="flex items-center justify-between pb-3 border-b border-amber-200/60">
                                    ...
                                    <span className="bg-amber-200 text-amber-900 px-2 py-0.5 rounded-full text-xs font-black">
                                        {cat.suspended.length}
                                    </span>
                                </div>

                                <div className="space-y-3 max-h-[600px] overflow-y-auto pr-1">
                                    {cat.suspended.length > 0 ? (
                                        cat.suspended.map(user => renderCard(user, 'suspended'))
                                    ) : (
                                        <div className="py-8 text-center text-slate-400 text-xs font-semibold italic">
                                            Niciun client suspendat în această secțiune.
                                        </div>
                                    )}
                                </div>
                            </div>

                            {/* Column 3: Clienți Arhivați */}
                            <div className="bg-slate-100/70 rounded-xl border border-slate-300/80 p-4 space-y-3">
                                ...
                                <div className="space-y-3 max-h-[600px] overflow-y-auto pr-1">
                                    {cat.archived.length > 0 ? (
                                        cat.archived.map(user => renderCard(user, 'archived'))
                                    ) : (
                                        <div className="py-8 text-center text-slate-400 text-xs font-semibold italic">
                                            Niciun client arhivat în această secțiune.
                                        </div>
                                    )}
                                </div>
                            </div>
