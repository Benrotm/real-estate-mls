...
export default function BulkImportOlxPage() {
            ...
            .on(
                ...
                (payload) => {
                    if (payload.new.status === 'completed' || payload.new.status === 'failed') {
                        ...
                        setMessage(payload.new.status === 'completed' ? 'OLX extraction run completed successfully!' : 'A critical error crashed the running job.');
                    }
                }
            )
            .subscribe();
    ...
    const handleRunBulkScrape = async (e: React.FormEvent) => {
        ...
        if (!url || !url.includes('olx.ro')) {
            setStatus('error');
            setMessage('Please enter a valid OLX.ro category URL.');
            return;
        }

        setIsLoading(true);
        setStatus('running');
        setMessage('Initializing background OLX scraper server...');
        setLogs([]);
        setActiveJobId(null);
        ...
        try {
            ...
            setLogs([{ id: 'init', message: 'Establishing SECURE link to Render Microservice (OLX Mode)...', log_level: 'info', created_at: new Date().toISOString() }]);
            ...
            if (!res.ok) {
                ...
                throw new Error(data.error || 'Failed to start OLX bulk scraper');
            }

            setMessage(`OLX Crawler dispatched! Listening for live logs...`);

        } catch (err: any) {
            console.error('OLX Bulk Import Error:', err);
            setStatus('error');
            setIsLoading(false);
            setMessage(err.message || 'An unexpected error occurred while starting the OLX crawler.');
        }
    };
    ...
    const handleStopScrape = async () => {
        ...
        try {
            ...
            setLogs((prev) => [...prev, { id: 'halt', message: 'STOP SIGNAL SENT. Waiting for OLX scraper to finish current cycle and exit.', log_level: 'warn', created_at: new Date().toISOString() }]);
            setIsLoading(false);
            setStatus('error');
            setMessage('OLX Import Halted by User.');
        } catch (e) {
            console.error(e);
        }
    };
        ...
        <div className="p-8 max-w-5xl mx-auto space-y-8">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-3xl font-bold text-slate-900 flex items-center gap-3">
                        ...
                        OLX.ro Import Terminal
                    </h1>
                    <p className="text-slate-500 mt-2">
                        Deploy an automated OLX.ro crawler to harvest listings with JSON-LD extraction &amp; Google Maps coordinates.
                    </p>
                </div>
                <Link
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <div className="lg:col-span-5 space-y-6">
                    <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                        ...
                        <div className="p-6">
                            <form onSubmit={handleRunBulkScrape} className="space-y-5">
                                <div>
                                    <label className="block text-sm font-bold text-slate-700 mb-2">OLX Category Page URL</label>
                                    <input
                                        type="url"
                                        required
                                        placeholder="https://www.olx.ro/imobiliare/apartamente-garsoniere-de-vanzare/timis/"
                                        value={url}
                                        onChange={(e) => setUrl(e.target.value)}
                                        className="w-full px-4 py-3 rounded-xl border-2 border-slate-200 focus:border-orange-500 focus:ring-4 focus:ring-orange-500/20 transition-all font-medium placeholder-slate-400 bg-slate-50 focus:bg-white text-sm"
                                        disabled={isLoading || status === 'running'}
                                    />
                                    <p className="text-xs text-slate-400 mt-1">
                                        Example: https://www.olx.ro/imobiliare/apartamente-garsoniere-de-vanzare/timis/
                                    </p>
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <input
                                            ...
                                            className="w-full px-4 py-3 rounded-xl border-2 border-slate-200 focus:border-orange-500 focus:ring-4 focus:ring-orange-500/20 transition-all font-medium bg-slate-50 focus:bg-white text-sm"
                                            disabled={isLoading || status === 'running'}
                                        />
                                    </div>
                                    <div>
                                ...
                                {status === 'running' || isLoading ? (
                                    ...
                                    >
                                        ...
                                        Halt OLX Crawler Immediately
                                    </button>
                                ) : (
                                    <button
                                        type="submit"
                                        disabled={!url}
                                        className="w-full flex items-center justify-center gap-2 px-6 py-4 bg-orange-600 text-white rounded-xl font-bold hover:bg-orange-700 hover:-translate-y-0.5 transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-[0_4px_12px_rgba(234,88,12,0.2)] hover:shadow-[0_8px_24px_rgba(234,88,12,0.3)]"
                                    >
                                        <Play className="w-5 h-5 text-orange-200" />
                                        Launch OLX Scraper
                                    </button>
                                )}
                            </form>
                    ...
                    {/* OLX Features Info */}
                    <div className="bg-orange-50/50 border border-orange-100 rounded-xl p-4 space-y-2 text-sm text-slate-700">
                        <h3 className="font-bold text-slate-900 flex items-center gap-2">
                            <Globe className="w-4 h-4 text-orange-500" /> OLX Scraper Features
                        </h3>
                        <ul className="space-y-1 text-xs text-slate-600">
                            <li>✓ JSON-LD structured data extraction (title, price, images)</li>
                            <li>✓ Google Maps link parsing for exact lat/lng coordinates</li>
                            <li>✓ Parameter extraction (area, floor, rooms, compartimentare)</li>
                            <li>✓ Phone number extraction from reveal buttons</li>
                            <li>✓ Auto-deduplication against existing listings</li>
                        </ul>
                    </div>
                </div>

                {/* VISUAL TERMINAL PANEL */}
                <div className="lg:col-span-7 flex flex-col h-[600px] bg-[#0a0f1c] rounded-2xl border border-orange-900/30 shadow-2xl overflow-hidden font-mono text-sm">
                    <div className="h-10 bg-slate-800/80 border-b border-slate-700 flex items-center px-4 justify-between shrink-0">
                        <div className="flex gap-2 items-center">
                            ...
                            <span className="ml-3 text-xs text-slate-400 font-medium flex items-center gap-2">
                                <Terminal className="w-3.5 h-3.5" /> OLX.ro Scraper — Render Microservice Live SSH
                            </span>
                        </div>
                        {activeJobId && (
                            <span className="text-xs text-orange-400/80 animate-pulse bg-orange-500/10 px-2 py-0.5 rounded">
                                ● REALTIME ACTIVE
                            </span>
                        )}
                    </div>
                    ...
                    <div className="flex-1 p-5 overflow-y-auto w-full scrollbar-thin scrollbar-thumb-slate-700 scrollbar-track-transparent">
                        {logs.length === 0 && status === 'idle' && (
                            <div className="text-slate-500/50 flex flex-col items-center justify-center h-full max-w-sm mx-auto text-center">
                                ...
                                <p>OLX scraper standing by. Enter a category URL and launch to view remote proxy logs.</p>
                            </div>
                        )}
            ...
            <div className="bg-orange-50/50 border border-orange-100 rounded-xl p-5 flex gap-4">
                ...
                <div className="text-sm text-slate-700">
                    <strong className="block mb-1 text-slate-900">OLX.ro Render Proxy Pipeline Connected</strong>
                    This terminal streams logs directly from the Render.com Virtual Private Server via Supabase WebSockets.
                    The OLX scraper uses separate code from Publi24 — JSON-LD extraction, Google Maps coordinates, and OLX-specific parameter parsing.
                    You can safely navigate away; the background process will continue.
                </div>
            </div>
        </div>
    );
