...
import {
    ...
    Users, Target, Zap, Clock, Calculator, Search, Filter
} from 'lucide-react';
...
export default function MarketAnalyticsClient({ role, userRole }: { role: string; userRole: string }) {
    ...
    useEffect(() => {
        ...
        async function loadData() {
            setLoading(true);
            try {
                const result = await getMarketAnalyticsData({ timeRange, propertyType, category, city });
                if (isMounted) {
                    setData(result);
    ...
    }, [timeRange, propertyType, category, city]);
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        ...
                        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 relative overflow-hidden group">
                            ...
                            <div className="flex items-center gap-4 mb-4">
                                ...
                                <div>
                                    <p className="text-slate-400 text-sm font-medium">Active Supply</p>
                                    <h3 className="text-3xl font-bold text-white">{formatNum(animatedKpis.totalSupply)}</h3>
                                </div>
                            </div>
                            <p className="text-xs text-slate-500">Properties currently matching filters</p>
                        </div>
                        ...
                        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 relative overflow-hidden group">
                            ...
                            <div className="flex items-center gap-4 mb-4">
                                ...
                                <div>
                                    <p className="text-slate-400 text-sm font-medium">Lead Demand</p>
                                    <h3 className="text-3xl font-bold text-white">{formatNum(animatedKpis.totalDemand)}</h3>
                                </div>
                            </div>
                            <p className="text-xs text-slate-500">Active inquiries and leads in range</p>
                        </div>
                        ...
                        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 relative overflow-hidden group">
                            ...
                            <div className="flex items-center gap-4 mb-4">
                                ...
                                <div>
                                    <p className="text-slate-400 text-sm font-medium">Average Price</p>
                                    <h3 className="text-3xl font-bold text-white">€{formatNum(animatedKpis.avgPrice)}</h3>
                                </div>
                            </div>
                            <p className="text-xs text-slate-500">Based on recent sales/listings. €{formatNum(data.kpis.avgPricePerSqm)}/m²</p>
                        </div>

                        {/* KPI 4: Market Temp (Gauge Alternative) */}
                        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 relative overflow-hidden group">
                            <div className={`absolute -right-4 -top-4 w-24 h-24 rounded-full blur-xl transition-all ${getMarketStatus(data.kpis.marketTemperature).bg.replace('10', '20')}`} />
                            <div className="flex items-center gap-4 mb-4">
                                <div className={`w-12 h-12 border rounded-xl flex items-center justify-center ${getMarketStatus(data.kpis.marketTemperature).bg} ${getMarketStatus(data.kpis.marketTemperature).color.replace('text', 'border')}/30`}>
                                    <Activity className={`w-6 h-6 ${getMarketStatus(data.kpis.marketTemperature).color}`} />
                                </div>
                                <div>
                                    <p className="text-slate-400 text-sm font-medium">Market Temp</p>
                                    <h3 className="text-3xl font-bold text-white">{animatedKpis.marketTemperature}<span className="text-lg text-slate-500 font-normal">/100</span></h3>
                                </div>
                            </div>
                            <p className={`text-xs font-semibold ${getMarketStatus(data.kpis.marketTemperature).color}`}>{getMarketStatus(data.kpis.marketTemperature).label}</p>
                        </div>
                    </div>
