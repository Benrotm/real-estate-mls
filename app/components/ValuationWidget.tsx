...
import { checkValuationUnlock, unlockValuation, getUserCredits } from '@/app/lib/actions/credits';
...
export default function ValuationWidget({ property }: ValuationWidgetProps) {
    ...
    // Collapsible & Credit states
    const [isOpen, setIsOpen] = useState(false);
    const [isUnlocked, setIsUnlocked] = useState(false);
    const [isLoggedIn, setIsLoggedIn] = useState(false);
    const [userCredits, setUserCredits] = useState(0);
    const [featureCost, setFeatureCost] = useState(1);
    ...
    // Check unlock status & credit cost
    useEffect(() => {
        async function checkUnlockAndCost() {
            ...
            try {
                ...
                // Check if already unlocked (or owner/admin)
                const unlockRes = await checkValuationUnlock(property.id);
                ...
                // Fetch current user credits & cost of the feature
                const [creditsRes, costsRes] = await Promise.all([
                    getUserCredits(),
                    getFeatureCosts()
                ]);
                ...
                if (costsRes && costsRes.costs) {
                    const cost = costsRes.costs['valuation_reports'] !== undefined ? costsRes.costs['valuation_reports'] : 1;
                    setFeatureCost(cost);
                }
            } catch (err) {
                console.error("Error checking unlock and costs:", err);
            } finally {
                setIsChecking(false);
            }
        }
    ...
    // Load valuation data when unlocked
    useEffect(() => {
        async function loadValuation() {
            if (!property?.id || !isUnlocked) return;
            setLoading(true);
            try {
    ...
    const handleUnlock = async () => {
        ...
        setIsUnlocking(true);
        try {
            const res = await unlockValuation(property.id, property.title || 'Proprietate');
            if (res.success) {
                setIsUnlocked(true);
                setUserCredits(res.remaining || 0);
            } else {
                alert(res.error || 'Failed to unlock valuation.');
            }
        } catch (err: any) {
            console.error('Error unlocking valuation:', err);
            alert(err.message || 'Error occurred during unlock.');
        } finally {
    ...
    const supplyScatterData = valuation ? [
    ...
    const chartMaxComp = valuation ? Math.max(medianPrice, property.price, ...((valuation.comparables || []).map((c: any) => Number(c.sold_price || 0)))) : 0;
    const chartMaxSupply = valuation ? Math.max(medianSupplyPrice, property.price, ...((valuation.currentSupply || []).map((s: any) => Number(s.price || 0)))) : 0;
    ...
    return (
        <div className="bg-white rounded-3xl border border-gray-200 shadow-xl overflow-hidden my-8 scroll-mt-24 relative" id="valuation">
            {/* Header */}
            <div 
                className="bg-gray-50 p-6 flex justify-between items-center text-slate-900 border-b border-gray-200 cursor-pointer select-none group"
            ...
            >
                ...
                <div className="flex items-center gap-4">
                    {/* Status Badge */}
                    {isChecking ? (
                        <span className="text-[10px] font-black text-slate-400 bg-slate-100 px-3 py-1.5 rounded-full uppercase tracking-wider animate-pulse">
                            Checking...
                        </span>
                    ) : !isLoggedIn ? (
                        <span className="text-[10px] font-black text-amber-600 bg-amber-50 border border-amber-100 px-3 py-1.5 rounded-full uppercase tracking-wider flex items-center gap-1">
                            <Lock className="w-3.5 h-3.5" /> Login to view
                        </span>
                    ) : isUnlocked ? (
                        <span className="text-[10px] font-black text-emerald-600 bg-emerald-50 border border-emerald-100 px-3 py-1.5 rounded-full uppercase tracking-wider flex items-center gap-1">
                            <CheckCircle className="w-3.5 h-3.5" /> Unlocked
                        </span>
                    ) : (
                        <span className="text-[10px] font-black text-indigo-600 bg-indigo-50 border border-indigo-100 px-3 py-1.5 rounded-full uppercase tracking-wider flex items-center gap-1">
                            <Lock className="w-3.5 h-3.5" /> Cost: {featureCost} CR
                        </span>
                    )}

                    {/* Collapse Button */}
                    <div className="w-10 h-10 rounded-xl bg-white border border-gray-200 flex items-center justify-center text-slate-400 group-hover:text-slate-600 transition-colors">
            ...
            {isOpen && (
                <div className="border-t border-gray-200">
                    {isChecking ? (
                        <div className="p-12 text-center text-gray-500 animate-pulse font-bold">Verificare acces...</div>
                    ) : !isLoggedIn ? (
                        /* Not Logged In Prompt */
                        <div className="p-12 text-center bg-slate-50 flex flex-col items-center justify-center">
                            <div className="w-16 h-16 bg-slate-200/50 rounded-2xl flex items-center justify-center mb-4 text-slate-500">
                                <Lock className="w-8 h-8" />
                            </div>
                            <h4 className="text-xl font-bold text-slate-900 mb-2 font-sans">Deblochează Evaluarea Inteligentă</h4>
                            <p className="text-slate-500 text-sm max-w-sm mb-6 leading-relaxed font-sans font-medium">
                                Vă rugăm să vă autentificați în cont pentru a accesa estimarea de piață lifestyle-adjusted, istoricul de vânzări și comp-urile active.
                            </p>
                            <Link
                                href={`/auth/login?redirectTo=/properties/${property.id}`}
                                className="bg-indigo-600 text-white py-3 px-8 rounded-xl font-bold text-sm hover:bg-indigo-500 transition-all shadow-lg shadow-indigo-600/20"
                            >
                                Conectează-te
                            </Link>
                        </div>
                    ) : !isUnlocked ? (
                        /* Logged In but Locked Prompt */
                        <div className="p-12 text-center bg-slate-50 flex flex-col items-center justify-center relative overflow-hidden">
                            <div className="absolute top-0 right-0 w-48 h-48 bg-indigo-500/5 blur-3xl rounded-full pointer-events-none" />
                            
                            <div className="w-16 h-16 bg-indigo-500/10 rounded-2xl border border-indigo-100 flex items-center justify-center mb-6 relative shrink-0">
                                <div className="absolute inset-0 bg-indigo-500/20 blur-xl rounded-full" />
                                <Lock className="w-8 h-8 text-indigo-600 relative z-10" />
                            </div>
                            <h4 className="text-2xl font-black text-slate-900 mb-2 tracking-tight font-sans">Deblochează Raportul Complet</h4>
                            <p className="text-slate-500 text-sm max-w-sm mb-6 leading-relaxed font-sans font-medium">
                                Vezi cum calitatea aerului, potențialul solar și tranzacțiile comparabile din zonă afectează valoarea reală a acestei proprietăți.
                            </p>

                            <div className="flex flex-wrap gap-4 items-center justify-center bg-white border border-gray-200 p-4 rounded-2xl shadow-sm mb-8 max-w-md w-full">
                                <div className="flex-1 text-center border-r border-gray-100 last:border-none px-4">
                                    <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-0.5">Cost Deblocare</span>
                                    <span className="text-lg font-black text-indigo-600 font-mono">{featureCost} CR</span>
                                </div>
                                <div className="flex-1 text-center border-r border-gray-100 last:border-none px-4">
                                    <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-0.5">Balanță Credite</span>
                                    <span className="text-lg font-black text-slate-900 font-mono">{userCredits} CR</span>
                                </div>
                            </div>

                            {userCredits >= featureCost ? (
                                <button
                                    type="button"
                                    onClick={handleUnlock}
                                    disabled={isUnlocking}
                                    className="bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white py-4 px-10 rounded-2xl font-black uppercase tracking-widest text-xs transition-all shadow-xl shadow-indigo-600/20 hover:scale-[1.02] active:scale-[0.98]"
                                >
                                    {isUnlocking ? 'Se deblochează...' : `Deblochează cu ${featureCost} Credite`}
                                </button>
                            ) : (
                                <div className="flex flex-col items-center gap-4 w-full max-w-sm">
                                    <div className="text-xs font-bold text-rose-500 flex items-center gap-1.5 bg-rose-50 px-4 py-2.5 rounded-xl border border-rose-100 w-full justify-center">
                                        <Info className="w-4 h-4" /> Fonduri insuficiente în cont
                                    </div>
                                    <Link
                                        href="/cont/plati"
                                        className="bg-slate-900 text-white hover:bg-slate-800 py-4 px-10 rounded-2xl font-black uppercase tracking-widest text-xs transition-all shadow-xl w-full text-center"
                                    >
                                        Alimentează Contul (Plăți)
                                    </Link>
                                </div>
                            )}
                        </div>
                        ...
                        /* Loading Data */
                        <div className="p-16 text-center text-slate-400 animate-pulse font-bold flex flex-col items-center justify-center gap-3">
                            <div className="w-8 h-8 border-4 border-indigo-600 border-t-transparent rounded-full animate-spin" />
                            <span>Calculare Estimare & Analiză de Piață...</span>
                        </div>
                    ) : !valuation ? (
                        <div className="p-12 text-center text-slate-500">Datele de evaluare nu au putut fi încărcate.</div>
                    ) : (
                        /* Unlocked Data Content */
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start mb-10">
                        {/* Left Column (Supply/Demand + Adjustments) */}
                        <div className="lg:col-span-2 space-y-8">
                            {/* Supply vs Demand Dynamics (Dark Theme) */}
                            <div className="bg-slate-900 border border-slate-800 p-8 rounded-2xl shadow-xl overflow-hidden relative group">
                            ...
                            {/* Market Adjustments (Relocated here to fill gap) */}
                            <div className="bg-white border border-gray-100 p-8 rounded-2xl shadow-sm space-y-4">
                                <h4 className="text-[10px] font-black text-slate-400 flex items-center gap-2 uppercase tracking-[0.2em] mb-4">
